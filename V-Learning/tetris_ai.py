...
def ai_play_search(model, max_games=100, is_gui_on=True):
    """
    AI playing with search algorithm
    Args:
        model: trained model
        max_games: maximum number of games to play
        is_gui_on: whether to show GUI (True for watching, False for evaluation)
    """
    max_steps_per_episode = 2000
    seed = None
    gui = Gui() if is_gui_on else None
    env = Game(seed=seed, height=0)
    env_gui = Game(gui=gui) if is_gui_on else None
    ...
    pause_time = 0.04 if is_gui_on else 0.0
    
    if is_gui_on:
        print("AI watching mode started. Press 'Q' to quit, 'P' to pause/resume.")
    else:
        print(f"AI evaluation mode started. Running {max_games} games without GUI...")
        ...
        for step in range(max_steps_per_episode):
            ...
            for m in moves_buffer:
                if is_gui_on:
                    ...
                    # Check for keyboard input to quit or pause
                    if gui and hasattr(gui, 'check_quit'):
                        if gui.check_quit():
                            print("Quitting AI watching mode...")
                            return
                        if gui.check_pause():
                            paused = not paused
                    ...
                    if paused:
                        while paused:
                            time.sleep(0.1)
                            if gui and hasattr(gui, 'check_pause'):
                                if gui.check_pause():
                                    paused = not paused
            ...
            if env.is_done() or step >= max_steps_per_episode - 1:
                ...
                game_length = game_end_time - game_start_time
                
                # Collect detailed statistics
                score = env.current_state.score
                lines = env.current_state.lines
                t_spins = env.current_state.t_spins[0]  # Use first element of t_spins list
                combo = env.current_state.combo
                
                total_score += score
                total_lines += lines
                total_t_spins += t_spins
                total_combos += combo
                
                scores_list.append(score)
                lines_list.append(lines)
                t_spins_list.append(t_spins)
                combos_list.append(combo)
                game_lengths.append(game_length)
                
                if is_gui_on:
                    print('episode #{}:   score:{}   lines:{}   t-spins:{}   combo:{}   time:{:.1f}s'.format(
                        episode_count, score, lines, t_spins, combo, game_length))
                else:
                    print(f'Game {episode_count}/{max_games}: Score={score}, Lines={lines}, T-spins={t_spins}, Combo={combo}, Time={game_length:.1f}s')
                ...
                intermediate_results = {
                    ...
                    'total_t_spins': total_t_spins,
                    'total_combos': total_combos,
                    'avg_score': total_score / episode_count,
                    'avg_lines': total_lines / episode_count,
                    'avg_t_spins': total_t_spins / episode_count,
                    'avg_combos': total_combos / episode_count,
                    'current_game': {
                        'score': score,
                        'lines': lines,
                        't_spins': t_spins,
                ...
                import json
                intermediate_filename = FOLDER_NAME + f'evaluation_intermediate_{int(time.time())}.json'
                with open(intermediate_filename, 'w') as f:
                    json.dump(intermediate_results, f, indent=2)
                
                # Also append to record.txt for real-time tracking
                record_text = f'Game {episode_count}/{max_games} | Score: {score} | Lines: {lines} | T-spins: {t_spins} | Combo: {combo} | Time: {game_length:.1f}s | Running Avg Score: {total_score/episode_count:.2f} | Running Avg Lines: {total_lines/episode_count:.2f}\n'
                append_record(record_text)
                break

    # Calculate final statistics
    avg_score = total_score / max_games
    avg_lines = total_lines / max_games
    avg_t_spins = total_t_spins / max_games
    avg_combos = total_combos / max_games
    ...
    # Save to JSON file for easy analysis
    import json
    results_filename = FOLDER_NAME + f'evaluation_results_{int(time.time())}.json'
    ...
    print('='*60)
    print(f'Games played: {episode_count}')
    print(f'Average score: {avg_score:.2f}')
    print(f'Average lines cleared: {avg_lines:.2f}')
    print(f'Average T-spins: {avg_t_spins:.2f}')
    print(f'Average combos: {avg_combos:.2f}')
    print(f'Average game length: {avg_game_length:.2f}s')
    print(f'Best score: {max_score}')
    print(f'Best lines: {max_lines}')
    print(f'Total evaluation time: {sum(game_lengths):.1f}s')
    print(f'Results saved to: {results_filename}')
    print('='*60)
    
    # Also append to record.txt for consistency
    record_text = f'\nEVALUATION RESULTS | Games: {episode_count} | Avg Score: {avg_score:.2f} | Avg Lines: {avg_lines:.2f} | Avg T-spins: {avg_t_spins:.2f} | Avg Combos: {avg_combos:.2f} | Avg Time: {avg_game_length:.2f}s | Best Score: {max_score} | Best Lines: {max_lines}\n'
    append_record(record_text)
...
def get_data_from_playing_cnn2d(model_filename, target_size=8000, max_steps_per_episode=2000, proc_num=0,
    ...
    if not model_filename.endswith('.keras') and not model_filename.endswith('.h5'):
...
if __name__ == "__main__":
    ...
    elif MODE == 'ai_player_evaluation':
        model_load = keras.models.load_model(FOLDER_NAME + 'whole_model/outer_{}.keras'.format(OUT_START))
        ai_play_search(model_load, is_gui_on=True, max_games=3)  # Run 3 games with GUI for monitoring
